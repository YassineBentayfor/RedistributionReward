...
async function main() {
  ...
  const accountAId = AccountId.fromString(process.env.ACCOUNT_A_ID);
  const accountAKey = PrivateKey.fromStringECDSA(process.env.ACCOUNT_A_PRIVATE_KEY);
  const accountBId = AccountId.fromString(process.env.ACCOUNT_B_ID);
  const accountBKey = PrivateKey.fromStringECDSA(process.env.ACCOUNT_B_PRIVATE_KEY);
  const accountCId = AccountId.fromString(process.env.ACCOUNT_C_ID);
  const accountCKey = PrivateKey.fromStringECDSA(process.env.ACCOUNT_C_PRIVATE_KEY);
  const contractId = AccountId.fromString(
    process.env.REWARD_DISTRIBUTION_CONTRACT_ID
  );

  const accountAAddress = process.env.ACCOUNT_A_ADDRESS_ETHER;
  const accountBAddress = process.env.ACCOUNT_B_ADDRESS_ETHER;
  const accountCAddress = process.env.ACCOUNT_C_ADDRESS_ETHER;
